  // components/Home.jsx (Updated - Player removed)
  import React, { useEffect, useState, useRef, useCallback } from 'react';
  import {
    Play, Pause, SkipBack, SkipForward, Volume2, VolumeX,
    Heart, Shuffle, Repeat, ChevronDown, Music
  } from 'lucide-react';
  import api from '../api';
  ...
  /* ─── Track Row Component ─── */
  const TrackRow = ({ track, index, isActive, isPlaying, onPlay, onLike }) => (
  ...
  /* ══════════════════════════════════════════
    HOME — Main Component (Without Player)
  ══════════════════════════════════════════ */
  export default function Home() {
    ...
    // Player state removed - now using global MusicPlayer component
    ...
    const handleOpenAlbum = async (album) => {
      try {
        ...
        if (tracks.length) {
          // Handle album playback - you can integrate with global player state here
            ...
            {musics.length ? (
              musics.map((track, index) => (
                <TrackRow
                  ...
                  isActive={false}
                  isPlaying={false}
                  onPlay={() => console.log('Play track:', track)}
                  onLike={handleLike}
                />
              ))
            ) : (
